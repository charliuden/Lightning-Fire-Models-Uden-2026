...
data {
  ...
  vector[N] CXP;            // Climate variable 1
}
...
model {
  ...
  alpha = exp(a_alpha + b_alpha * CXP);
  beta = exp(a_beta + b_beta * CXP);
...
generated quantities {
  ...
  for (n in 1:N) {
    ...
    real alpha_n = exp(a_alpha + b_alpha * CXP[n]);
    real beta_n = exp(a_beta + b_beta * CXP[n]);
